...
import {
  GET_COURTS,
  GET_COURTS_BY_ID,
  GET_COURTS_BY_DIOCESE,
} from "@/graphql/queries/courtListingQuery";
import type {
  CourtListingBlock as CourtListingBlockProps,
  LocalCourt,
} from "@/payload-types";
...
function convertToAnniversary(date: string) {
  ...
  const hasOccurredThisYear =
    ...
    (today.getMonth() === institutedDate.getMonth() &&
      today.getDate() >= institutedDate.getDate());
...
export const CourtListing: React.FC<CourtListingBlockProps> = ({
  ...
  const variables = useMemo(() => {
    ...
    if (
      selectionType === "diocese" &&
      selectedDiocese &&
      typeof selectedDiocese === "object"
    ) {
      return {
  ...
  const columns = [
    ...
    },
    {
      ...
      accessor: "instituted" as keyof LocalCourt,
      render: (_: any, row: { instituted: string }) => (
        <p>{convertToAnniversary(row.instituted)}</p>
      ),
    },
    {
      header: "Court Regent",
      ...
      render: (_: any, row: LocalCourt) =>
        row.courtOfficers?.courtRegent &&
        typeof row.courtOfficers.courtRegent === "object" ? (
          <div>
  ...
  if (error) {
    console.error(error);
    return (
      <div className="container my-20 text-red-500">Error loading courts.</div>
    );
  }
