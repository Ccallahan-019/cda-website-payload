...
export const SideBarBlock: React.FC<SideBarBlockProps> = (props) => {
  ...
  return (
    <div className="container my-16 sm:my-20">
      <div
        className={`py-10 px-4 sm:px-6 rounded-xl bg-background/60 backdrop-blur-sm shadow-xl from-slate-200 to-40% ${left ? "md:bg-gradient-to-r" : "md:bg-gradient-to-l"}`}
      >
        <div className={`hidden md:grid grid-cols-12 gap-8 min-h-[50vh]`}>
          <main
            className={`h-full flex items-center py-8 col-span-9 ${left ? "pr-8 xl:pr-0 col-start-4 xl:col-start-4" : "pl-8 xl:pl-0"}`}
          >
            <div>
              {activeSection && (
          ...
          <div
            className={`relative col-span-3 ${left ? "row-start-1 col-start-1" : ""}`}
          >
            <SideBar
              alignment={alignment}
              sections={sections}
