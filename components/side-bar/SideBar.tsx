...
type Props = {
  ...
  onClick: Dispatch<
    SetStateAction<SideBarBlock["sections"][number] | undefined>
  >;
};
...
export default function SideBar({
  ...
  return (
    <aside className="h-full flex flex-col justify-center">
      <div
        className={`space-y-12 ${left ? "flex flex-col text-right items-end" : ""}`}
      >
        {sections &&
          sections.length > 0 &&
          sections.map((section, index) => (
            ...
            >
              <div className="relative flex items-center">
                <h3 className="text-3xl">{section.sectionHeading}</h3>
              </div>
            </motion.div>
          ))}
      </div>
