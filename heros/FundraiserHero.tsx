...
export const FundraiserHero: React.FC<{
  fundraiser: Fundraiser
}> = ({ fundraiser }) => {
  const { heroImage, fundraiserName, fundraiserType } = fundraiser
      ...
      <div className="container z-10 relative lg:grid lg:grid-cols-[1fr_64rem_1fr] text-white pb-8">
        <div className="col-start-1 col-span-1 md:col-start-2 md:col-span-2">
          <div className="uppercase text-sm mb-6">
            {fundraiserType && <p>{`${fundraiserType} Fundraiser`}</p>}
          </div>

          <div className="">
            <h1 className="mb-6 text-3xl md:text-5xl lg:text-6xl">{fundraiserName}</h1>
          </div>
        </div>
      </div>
